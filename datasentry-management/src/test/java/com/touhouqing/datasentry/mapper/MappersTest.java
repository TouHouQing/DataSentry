...
public class MappersTest {
	...
	private Long createAgent(String name) {
		Agent datasentry_agent = Agent.builder()
			.name(name)
			.description("for fk")
			.avatar("a")
			.status("draft")
			...
			.build();
		agentMapper.insert(datasentry_agent);
		return datasentry_agent.getId();
	}
	...
	public void testAgentMapper() {
		...
		Agent datasentry_agent = Agent.builder()
			...
			.updateTime(LocalDateTime.now().withNano(0))
			.build();
		int insert = agentMapper.insert(datasentry_agent);
		Assertions.assertEquals(1, insert);
		Agent findById = agentMapper.findById(datasentry_agent.getId());
		Assertions.assertEquals(datasentry_agent, findById);
		List<Agent> findByStatus = agentMapper.findByStatus("test");
		Assertions.assertEquals(List.of(datasentry_agent), findByStatus);
		List<Agent> searchByKeyword = agentMapper.searchByKeyword("test");
		Assertions.assertEquals(List.of(datasentry_agent), searchByKeyword);
		datasentry_agent.setName("test2");
		int update = agentMapper.updateById(datasentry_agent);
		Assertions.assertEquals(1, update);
		agentMapper.deleteById(datasentry_agent.getId());
