...
CREATE TABLE IF NOT EXISTS `datasentry_semantic_model` (
  ...
  `agent_id` BIGINT(11) NOT NULL COMMENT '关联的智能体ID',
  `datasource_id` BIGINT(11) NOT NULL COMMENT '关联的数据源ID',
...
CREATE TABLE IF NOT EXISTS `datasentry_agent_knowledge` (
  `id` BIGINT(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID, 用于内部关联',
  `agent_id` BIGINT(11) NOT NULL COMMENT '关联的智能体ID',
...
CREATE TABLE IF NOT EXISTS datasentry_cleaning_policy_release_ticket (
  ...
  publish_mode VARCHAR(32) DEFAULT 'FULL' COMMENT '发布模式：FULL/GRAY',
  gray_ratio DECIMAL(5,4) DEFAULT NULL COMMENT '灰度比例',
  experiment_name VARCHAR(128) DEFAULT NULL COMMENT '实验名称',
